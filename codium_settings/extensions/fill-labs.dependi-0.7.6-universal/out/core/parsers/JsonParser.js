...
class JsonParser {
    constructor(depsKey, devDepsKey) {
        this.depsKey = depsKey;
        this.devDepsKey = devDepsKey;
        this.state = new State();
    }
    parse(doc) {
        const pattern = Language_1.CurrentLanguageConfig === "npm" ? config_1.Settings.npm.ignoreLinePattern : config_1.Settings.php.ignoreLinePattern;
        for (let row = 0; row < doc.lineCount; row++) {
            let line = doc.lineAt(row);
            if ((0, utils_1.shouldIgnoreLine)(line, pattern)) {
                continue;
            }
    ...
    isDependencies(line) {
        ...
        return (line.text.substring(start, start + this.depsKey.length + 3) === `"${this.depsKey}":` ||
